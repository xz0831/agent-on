...
# Derive the expected route from the shipped haiku alias rather than naming a
# model here. Hardcoding it made this assertion a hidden dependency on the tier
# mapping: the 3.6 -> 3.8 migration and this task-handoff feature were each
# correct alone, but merging them broke this line, and git could not see the
# conflict because the two edits are textually far apart.
task_expected_route="$(jq -r '.aliases.haiku' "$prefix/config/claude-litellm/settings.json")"
[[ -n "$task_expected_route" && "$task_expected_route" != null ]] || { echo "FAIL: could not read haiku alias" >&2; exit 1; }
[[ "$(print -r -- "$task_plan" | jq -r .route)" == "$task_expected_route" ]] || { echo "FAIL: task handoff did not normalize preset route" >&2; exit 1; }
...
[[ "$task_launch_out" == *"launch:$task_expected_route:$task_worktree:"* ]] || { echo "FAIL: task worker did not launch concrete route in worktree" >&2; exit 1; }
