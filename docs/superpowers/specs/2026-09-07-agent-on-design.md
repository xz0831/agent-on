...
| D12 ⟲ | **Claude effort follows an explicit backend contract.** Splash and OpenRouter consume Claude's Anthropic `output_config.effort` directly. New oMLX Claude launches use a route-pinned loopback adapter which maps that field to `chat_template_kwargs.reasoning_effort`; missing effort preserves the template default and disabled thinking wins. Only documented per-model profiles are mapped, and each applied mapping has a metadata-only receipt. | The earlier direct-wire finding remains historically correct: oMLX ignored Claude's effort field. The 2026-09-19 adapter addresses that one gap without proxying native Splash/OpenRouter paths or inventing universal model levels. |
| D13 | **Machine-written state lives outside git.** `$XDG_STATE_HOME/agent-on/` (default `~/.local/state/agent-on/`) holds `routes.discovered.toml`, host-local `routes.local.toml` base-URL overrides, `observed.json`, `locks/observed.lock` (§7.1 — the `add` lock is `<checkout>/.routes.lock`, keyed by the resource it protects), `sessions/<session-id>.jsonl` (the per-run cost ledger — §11), `effort/<launch-id>.jsonl` (0600 prompt/header-free oMLX mapping receipts), `run/<launch-id>/` (per-launch credentials removed on exit — §7.1), the default isolated Claude config dir, and the 0600 `env` file. A Claude launch may explicitly select `--session-store native` to use the operator's normal `~/.claude` session store in place; neither store is Git data. `knowledge/` and `routes.toml` live in the checkout and are git-tracked. `AGENT_ON_STATE` overrides the state root for scratch runs. Storage rules: §7.1. | The review's day-one gap: rev 2 never said where the machine-written files live or which copy owns them. With D9 there is one code copy per host; machine state remains host-local. `sync` never dirties a tracked file; `learn` and `add` produce diffs the agent can commit. |
...
|---|---|---|---|---|
| `claude-on [--session-store isolated\|native] <route\|alias> [claude args…]` | bind Claude Code to the route and **spawn** it as a child (inherited stdio, SIGINT/SIGTERM forwarded, exit status propagated); after exit, read the session transcript when one exists and record the session summary. `isolated` is the default. `native` leaves `CLAUDE_CONFIG_DIR` unset so normal Claude Code sessions can be resumed or continued in place from `~/.claude`. Session rules: the launcher passes `--session-id <uuid>` unless the user passed `--session-id` (theirs is used) or `--resume`/`--continue` (the resumed session's file is read); if the user passed `--no-session-persistence` there is no transcript and `last_session` is recorded as `{ "skipped": "no-session-persistence" }` — read-back is best-effort, never a reason to refuse arguments | L1 L2 | L2 | `route.served` (re-probed with one ≤2 s GET; unreachable → skip + warning, launch proceeds — D6), `harness.env.clean`, `credential.not_in_child_env` |
...
- **Unknown local effort profiles are intentionally unsupported** (D12 ⟲). A discovered oMLX model with no documented profile preserves its ordinary request when there is no explicit CLI effort and records Claude's default/UI effort as un-applied. An explicit `--effort` receives a clear local error. `qualify` still measures observed thinking; `CLAUDE_CODE_MAX_OUTPUT_TOKENS` remains the independent output cap.
